...
func (s Service) HandlePublicThemes(c *gin.Context) {
	...
	themesList := make([]map[string]any, 0)
	for rows.Next() {
		...
		themesList = append(themesList, serializeTheme(row, false))
	}
	c.JSON(http.StatusOK, gin.H{"themes": themesList})
}
...
func (s Service) HandleMyThemes(c *gin.Context) {
	...
	for rows.Next() {
		...
		themesList = append(themesList, serializeTheme(row, true))
	}
	c.JSON(http.StatusOK, gin.H{"themes": themesList})
}
