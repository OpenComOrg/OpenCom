...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		InternalToken: strings.TrimSpace(os.Getenv("LINK_PREVIEW_INTERNAL_TOKEN")),
	...
	if len(cfg.InternalToken) < 16 {
		return Config{}, errors.New("LINK_PREVIEW_INTERNAL_TOKEN is required and must be at least 16 characters")
	}
...
func loadDotEnvIfPresent() {
	paths := []string{
		".env",
		filepath.Join("go-linkpreview", ".env"),
	}
